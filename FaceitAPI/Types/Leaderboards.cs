...
namespace FaceitAPI.Types
{
    public class Leaderboards : ApiBase, ILeaderboards
    {
        ...
        {
            return Get<LeaderboardPaging<Ranking>>(
                $"https://open.faceit.com/data/v4/leaderboards/championships/{championship_id}/groups/{group}?offset={offset}&limit={limit}");
        }

        public Paging<LeaderboardDetails> GetChampionshipLeaderboards(string championship_id, int offset = 0, int limit = 20)
        {
            return Get<Paging<LeaderboardDetails>>(
        ...
        {
            return Get<LeaderboardPaging<Ranking>>(
                $"https://open.faceit.com/data/v4/leaderboards/hubs/{hub_id}/seasons/{season}?offset={offset}&limit={limit}");
        }
    }
}